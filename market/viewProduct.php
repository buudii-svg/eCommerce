...
<div class="productsList">
             <?php
                $stmt = $con->prepare("SELECT Name, Image, Brand, Price, Quantity, Bdesc, Fdesc FROM products Where MarketUser = ?");
                $stmt->execute(array($_SESSION['Username']));
                $rows = $stmt->fetchAll();
                foreach($rows as $row){
                    echo '<div class="product">';
                    echo '<img src="./layout/img/'.$row['Image'].'"/>';
                    echo '<h2>'.$row['Name'].'</h2>';
                    echo '<span>'.$row['Brand'].'</span>';
                    echo '<span>Price: '.$row['Price'].'</span>';
                    echo '<span>Quantity: '.$row['Quantity'].'</span>';
                    echo '<span>Description: '.$row['Bdesc'].'</span>';
                    echo '</div>';
                }
